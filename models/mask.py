...
def random_bbox(
    img_shape=(256, 256), max_bbox_shape=(128, 128), max_bbox_delta=40, min_margin=20
):
...
def brush_stroke_mask(
    ...
    if isinstance(num_vertices, int):
        ...
        raise TypeError(
            "The type of num_vertices should be int"
    ...
    if isinstance(brush_width, tuple):
        ...
        raise TypeError(
            "The type of brush_width should be int"
...
def random_irregular_mask(
    ...
    if isinstance(length_range, int):
        ...
        raise TypeError(
            "The type of length_range should be int"
...
def get_file_mask(filename):
    mask = np.array(Image.open(filename))
    return np.asarray(mask[..., 0] == 0, dtype="uint8")[..., np.newaxis]
